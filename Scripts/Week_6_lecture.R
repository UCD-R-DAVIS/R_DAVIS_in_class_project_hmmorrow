...
ggplot(data=yearly_counts)+geom_line(aes=(x=year,y=n))
...
library(tigris)
install.packages(sf)
ca_counties=tigris::counties(state='MN', class='sf')
ca_counties
ggplot(data=ca_counties)+geom_sf(aes(fill=-ALAND))+theme_map()+scale_fill_continuous_tableau()
